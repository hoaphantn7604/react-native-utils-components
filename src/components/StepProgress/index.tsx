...
const StepProgress: React.FC<Props> = (props) => {
  const { data, activeColor, inActiveColor, selectIndex, textSize, selectColor, onSelectIndex } = props;
    ...
    return (
      <View style={[index !== 0 && { flex: 1 }]}>
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <TouchableOpacity onPress={() => onSelectIndex(index)}>
            <View
              style={[
                styles.radius,
                {
              ...
              {item.status && (
                <Image style={[styles.icon, { tintColor: index === selectIndex ? selectColor : activeColor }]} source={require('./icon/check.png')} />
              )}
            </View>
          </TouchableOpacity>
        </View>
  ...
  const renderText = (item: Item, index: number) => {
    return (
      <View
        ...
        <Text style={[styles.text, { fontSize: fontScale(textSize) }]}>
          {item.text}
        </Text>
      </View>
    );
